...
class SortingSheetSandbox(
    ...
    override fun update(msg: Msg, model: Model): UpdateResult = when (msg) {
        ...
        is Msg.Ui.OnDefaultBtnClicked -> onDefaultBtnClicked(model)
    }

    private fun onOrderClicked(model: Model, msg: Msg.Ui.OnOrderClicked): UpdateResult =
        ElmUpdate(model, commands = setOf(Cmd.UpdateOrderState(msg.order)))

    private fun onSortSelected(model: Model, msg: Msg.Ui.OnSortSelected): UpdateResult =
        ElmUpdate(model, commands = setOf(Cmd.UpdateSortState(msg.sort)))

    private fun onCheckboxClicked(model: Model, msg: Msg.Ui.OnCheckboxClicked): UpdateResult =
        ElmUpdate(model, commands = setOf(Cmd.UpdateCheckboxState(msg.isChecked)))

    private fun onDefaultBtnClicked(model: Model): UpdateResult =
        ElmUpdate(model, commands = setOf(Cmd.SetDefaultSortState))
}